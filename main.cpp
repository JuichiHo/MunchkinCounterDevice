...
void setup()
{
  ...
  else
  {
    ...
    //настройки
    settingsLoad();
    Serial.println(F("sett"));                    // DEBUG
    Serial.println(settings.black_theme_display); // DEBUG
    Serial.println(settings.contrastIndex);       // DEBUG
    //параметры манчкина
    munchkinLoad();
    Serial.println(F("munch"));      // DEBUG
    Serial.println(munchkin.level);  // DEBUG
    Serial.println(munchkin.gender); // DEBUG
    Serial.println(munchkin.bonus);  // DEBUG
...
void gameButtonState()
{
  ...
  // Serial.print("--");
  // Serial.print(L_btn_State);
  // Serial.print(R_btn_State);
  // Serial.print(UP_btn_State);
  // Serial.print(DOWN_btn_State);
  // Serial.print(OK_btn_State);
  // Serial.println("--");
  ...
  if (!L_btn_State && L_flag)
  {
    ...
    else
    {
      ...
      if (settings_index_x > 0)
      {
        ...
        Serial.println(settings_index_y); // DEBUG
      }
    ...
    if (!settings_trigger)
    {
      ...
      if (game_Pointer == 5)
      {
        if (animation_ok_button_y < 1)
        {
          ...
          Serial.println("go to settings-menu"); // DEBUG
        }
      }
    }
    else
    {
      ...
      if (settings_index_y == 3 && settings_index_x == 0)
      {
        if (animation_ok_button_y < 1)
        {
          ...
          Serial.println(F("go to gametable")); // DEBUG
        }
      }
      ...
      if (settings_index_y == 3 && settings_index_x == 2)
      {
        if (animation_ok_button_y < 1)
        {
          ...
          Serial.println(F("munch reset")); // DEBUG
        }
      }
    }
  }
...
void batteryManagementSystem()
{
  ...
  // Serial.print(bat_volt_f); // DEBUG
  // Serial.print(" mV |");    // DEBUG
  // Serial.print(capacity);   // DEBUG
...
void calibration()
{
  ...
  // Serial.println(real_const, 3);                    // общаемся с пользователем - выдaёт напряжение после калибровки
...
{ //функция чтения внутреннего опорного напряжения, универсальная (для всех ардуин)
  ...
  // Serial.print("Vcc ADC ");   //DEBUG
  // Serial.println(analog_val); //DEBUG

  // boolean test = digitalRead(A0);
  ...
  do
  {
    ...
    if (flag_can_work == false && device_sleep == false && sevice_menu_flag == false)
    {
      //------setup--------
      {
        ...
        Serial.println(F("---low battery mode---"));
        delay(50);
